module ecosys_forcing_mod
  ...
  type(tracer_read)   :: FeScavDOCr_input             ! namelist input for FeScavDOCr
...
contains
  ...
  subroutine ecosys_forcing_init(ciso_on, land_mask,                     &
    ...
    namelist /ecosys_forcing_data_nml/                                        &
         ...
         iron_flux_input, fesedflux_input, feventflux_input, FeScavDOCr_input, &
    ...
    call set_defaults_tracer_read(FeScavDOCr_input, file_varname='FeScavDOCr')
        ...
        select case (trim(marbl_req_interior_tendency_forcings(n)%metadata%varname))
          ...
          case ('Iron Sediment Flux')
            ...
            call interior_tendency_forcings(n)%add_forcing_field(                &
                          ...
                          rank=3, dim3_len=km, id=n)
          case ('Fe Scavenging DOCr Vent Flux')
            FeScavDOCr_ind = n
            call interior_tendency_forcings(n)%add_forcing_field(                &
                          field_source='file_time_invariant',                 &
                          marbl_varname=marbl_varname, field_units=units,     &
                          filename=FeScavDOCr_input%filename,                  &
                          file_varname=FeScavDOCr_input%file_varname,          &
                          unit_conv_factor=FeScavDOCr_input%scale_factor,      &
                          rank=3, dim3_len=km, id=n)
  ...
  subroutine forcing_init_post_processing(land_mask)
    ...
    do iblock = 1, nblocks_clinic
      ...
      if (FeScavDOCr_ind > 0) then
        associate (FeScavDOCr => interior_tendency_forcings(FeScavDOCr_ind)%field_1d)
          call add_subsurf_to_bottom(land_mask(:,:,iblock), FeScavDOCr(:,:,:,iblock), iblock)
        end associate
      endif
