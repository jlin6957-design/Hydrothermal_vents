...
! jkm mods to Kd
 ...
 !---------------------------------------------------------------------------------------------
 !---------- Increase Upper Ocean Mixing at mid-latitudes where missing processes should mix more
 !----------------------------------------------------------------------------------------------


    !---------------------------------------------------------------
    ! increase for unresolved tidal mixing/salinity driven mixing
    !---------------------------------------------------------------
    ...
    !---------------------------------------------------------------
      if  (k .LE. 20) then
        if ((TLOND(i,j,iblock) .ge. 180.0_r8) .and. (TLOND(i,j,iblock) .lt. 206.0_r8)) then   
      	  if ((TLATD(i,j,iblock) .gt. 56.0_r8) .and. (TLATD(i,j,iblock) .le. 74.0_r8)) then
      	        bckgrnd_vdc(i,j,k,iblock) = 2.0_r8
      	  endif
      	 endif
        if ((TLOND(i,j,iblock) .gt. 193.0_r8) .and. (TLOND(i,j,iblock) .lt. 206.0_r8)) then   
      	  if ((TLATD(i,j,iblock) .gt. 54.0_r8) .and. (TLATD(i,j,iblock) .le. 74.0_r8)) then
      	        bckgrnd_vdc(i,j,k,iblock) = 2.0_r8
      	  endif
      	 endif
      endif
	...
	!---------------------------------------------------------------
        !Bering Sea shelf / shelf break
        !---------------------------------------------------------------
        if ((TLOND(i,j,iblock) .ge. 180.0_r8) .and. (TLOND(i,j,iblock) .lt. 205.0_r8)) then   
      	  if ((TLATD(i,j,iblock) .gt. 56.0_r8) .and. (TLATD(i,j,iblock) .le. 66.0_r8)) then
      	        bckgrnd_vdc(i,j,k,iblock) = 3.0_r8
      	  endif
      	 endif
        if ((TLOND(i,j,iblock) .gt. 193.0_r8) .and. (TLOND(i,j,iblock) .lt. 205.0_r8)) then   
      	  if ((TLATD(i,j,iblock) .gt. 54.0_r8) .and. (TLATD(i,j,iblock) .le. 66.0_r8)) then
      	        bckgrnd_vdc(i,j,k,iblock) = 3.0_r8
      	  endif
      	 endif
	...
	!---------------------------------------------------------------
        !Kuril mixing
        !---------------------------------------------------------------
  	 if ((TLOND(i,j,iblock) .ge. 134.0_r8) .and. (TLOND(i,j,iblock) .lt. 156.0_r8)) then
  	   if ((TLATD(i,j,iblock) .ge. 44.0_r8) .and. (TLATD(i,j,iblock) .lt. 66.0_r8)) then
      	       bckgrnd_vdc(i,j,k,iblock) = 3.0_r8
	   endif
  	  endif
	...
	!---------------------------------------------------------------
        ...
        if ((TLOND(i,j,iblock) .ge. 118.0_r8) .and. (TLOND(i,j,iblock) .lt. 146.0_r8)) then      
          if ((TLATD(i,j,iblock) .ge. 36.0_r8) .and. (TLATD(i,j,iblock) .lt. 48.0_r8)) then
 		bckgrnd_vdc(i,j,k,iblock) = 5.0_r8
      	  endif
	endif
